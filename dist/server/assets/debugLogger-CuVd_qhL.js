import { l as logger, D as DEFAULT_MODEL, P as PROVIDER_LIST } from './server-build-1cRs_EZQ.js';
...
const getLogStore = () => {
  if (!logStore && typeof window !== "undefined") {
    try {
      import('./server-build-1cRs_EZQ.js').then(n => n.c).then(({ logStore: store }) => {
        logStore = store;
      }).catch(() => {
      });
    } catch {
...
class DebugLogger {
  ...
  async _getLogStoreLogs() {
    try {
      ...
      if (!store) {
        try {
          const { logStore: storeModule } = await import('./server-build-1cRs_EZQ.js').then(n => n.c);
